...
form.addEventListener("submit", (event) => {
  ...
  // Salva neste navegador. Para receber respostas online, veja o README.md.
  const respostas = JSON.parse(localStorage.getItem("confirmacoes-casamento") || "[]");
  const respostaExistente = respostas.findIndex((item) => item.nome.toLowerCase() === dados.nome.toLowerCase());
  if (respostaExistente >= 0) respostas[respostaExistente] = dados;
  else respostas.push(dados);
  localStorage.setItem("confirmacoes-casamento", JSON.stringify(respostas));
